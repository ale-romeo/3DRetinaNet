...
def train(args, net, train_dataset, val_dataset):
    ...
    # Change the optimizer and scheduler to train only CEM modules
    ...
    cem_params = [
        ...
        if any(k in name for k in ["cem_head", "ego_transformer", "ego_head"]) and p.requires_grad
    ]

    optimizer = torch.optim.AdamW(cem_params, lr=args.LR * 5)  # o lr=0.02
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.5)

    solver_print_str = "[CEM OPTIM] Training cem_head, ego_transformer, ego_head with lr = {:.6f}".format(optimizer.param_groups[0]['lr'])
