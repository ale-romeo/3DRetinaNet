...
def copy_source(source_dir):
    # Se esiste ed è un file, lo cancelliamo
    ...
    for dirpath, dirs, files in os.walk('./', topdown=True):
        # Ignora la cartella "source" stessa
        source_basename = os.path.basename(os.path.normpath(source_dir))
        if source_basename in dirs:
            dirs.remove(source_basename)
        for file in files:
            if file.endswith('.py'):
                src = os.path.normpath(os.path.join(dirpath, file))
                dst = os.path.normpath(os.path.join(source_dir, file))
